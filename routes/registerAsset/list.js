...
exports.get = function*() {
    ...
    for (var i = 0; i < trans.length; i++) {
        if (trans[i].specification.destination.tag === config.tag.register) {
    ...
    for (var i = 0; i < trans2.length; i++) {
        if (trans2[i].specification.destination.tag === config.tag.register) {
    ...
    console.log(data)
    yield this.render("/registerAsset/list", {
        data: data
    })
